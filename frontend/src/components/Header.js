...
const navLinks = [
  ...
  { to: '/rendezvous', label: 'Prendre RDV', icon: <EventAvailableIcon /> },
  { to: '/mes-rendezvous', label: 'Mes RDV', icon: <ListAltIcon /> },
  { to: '/messagerie', label: 'Messagerie', icon: <ChatIcon /> },
  { to: '/notifications', label: 'Notifications', icon: <NotificationsActiveIcon /> },
  { to: '/profil', label: 'Profil', icon: <AccountCircleIcon /> },
];
...
const Header = () => {
  ...
  const fetchUnreadMessages = async () => {
    ...
    try {
      ...
      console.log('Notifications de messages:', messageNotifications);
      setUnreadMessages(messageNotifications.length);
      console.log('Nombre de messages non lus:', messageNotifications.length);
    } catch (error) {
  ...
  const fetchUnreadNotifications = async () => {
    if (!token) {
      console.log('Pas de token disponible pour les notifications');
      return;
    }
    ...
    try {
      ...
      console.log('Réponse API notifications totales:', response.data);
      
      // Compter toutes les notifications non lues
      const totalUnread = response.data.notifications?.length || 0;
      console.log('Nombre total de notifications non lues:', totalUnread);
      ...
      <AppBar position="sticky" color="primary" elevation={4} sx={{ zIndex: 1201 }}>
        <Toolbar>
          ...
          <Typography
            ...
            component={Link}
            to="/dashboard"
            sx={{ flexGrow: 1, color: 'inherit', textDecoration: 'none', fontWeight: 700, letterSpacing: 1 }}
          >
            MedRendezVous
          </Typography>
          {/* Desktop navigation */}
      ...
      >
        ...
        <DialogContent>
          ...
          >
            ...
            Vous devrez vous reconnecter pour accéder à votre espace personnel.
          </DialogContentText>
        </DialogContent>
